...
<div class="sortbar-tablet">
  <!-- ROW 1: Main filters (sort, levels, tags, country, my channels) -->
  ...
  <!-- ROW 2: Actions (reset, hide watched, search) -->
  <div class="tablet-row-2">
    {#if filtersChanged}
      <button class="reset-filters-btn" type="button" on:click={handleResetFilters}>reset filters</button>
    {/if}
...
.tablet-row-1 { justify-content: flex-start; }
.tablet-row-2 { justify-content: flex-end; border-top: 1px solid #eee; margin-top: 0.1em; padding-top: 0.28em; }

.dropdown { position: relative; min-width: 110px; }
.dropdown-btn { padding: 0.44em 1.1em; font-size: 1.04em; border-radius: 12px; border: 1.2px solid #ececec; background: #f9f9f9; color: #1d1d1d; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 0.65em; min-width: 108px; transition: border 0.11s, background 0.11s;}
.dropdown-btn[aria-expanded='true'],
.dropdown-btn[aria-pressed='true'] { background: #f1f5fb; border: 1.2px solid #bbb; color: #1d1d1d;}
.hide-watched-btn .switch-slider { width: 36px; height: 20px; background: #e8e8e8; border-radius: 8px; position: relative; display: inline-block; transition: background 0.13s; margin-right: 0.65em; vertical-align: middle;}
.hide-watched-btn[aria-pressed='true'] .switch-slider { background: #fd2b23;}
.hide-watched-btn .switch-slider::before { content: ''; position: absolute; width: 15px; height: 15px; left: 2.2px; top: 2.2px; background: #fff; border-radius: 50%; transition: transform 0.13s, box-shadow 0.13s; box-shadow: 0 1px 2px #0002;}
.hide-watched-btn[aria-pressed='true'] .switch-slider::before { transform: translateX(14px); box-shadow: 0 1px 4px #fd2b2333;}
.switch-label-text { font-size: 1.07em; font-weight: 600; letter-spacing: 0.03em; color: inherit; }
.dropdown-content { position: absolute; z-index: 1000; background: #fff; border: 1.3px solid #e8e8e8; border-radius: 8px; box-shadow: 0 2px 18px #eee; min-width: 190px; padding: 0.9em 0.7em; top: 110%; left: 0; font-size: 1.04em; max-height: 370px; overflow-y: auto;}
.levels-list { display: flex; flex-direction: column; gap: 0.6em; margin: 0.3em 0 0.4em 0;}
.level-checkbox { display: flex; align-items: center; gap: 0.6em; font-size: 1.07em;}
.search-bar-container { display: flex; align-items: center; position: relative; gap: 0.7em; }
.search-input { transition: width 0.2s, opacity 0.2s, box-shadow 0.13s, border 0.13s; width: 178px; max-width: 50vw; order: -1; margin-right: 0.3em; opacity: 1; font-size: 1.04em; border-radius: 12px; border: 1.2px solid #ececec; background: #f9f9f9; color: #1d1d1d; font-weight: 500; padding: 0.42em 1.1em; box-shadow: 0 2px 8px #ececec60; outline: none; }
.search-input:focus { border: 1.2px solid #bbb; background: #f1f5fb; box-shadow: 0 2px 16px #bbb2;}
.search-toggle { z-index: 2; background: none; border: none; cursor: pointer; display: flex; align-items: center; padding: 0.3em; margin-left: 0; border-radius: 50%; transition: background 0.18s;}
.search-toggle:hover, .search-toggle:focus-visible { background: #e5f2fd;}
.reset-filters-btn { font-weight: 800; color: #e93c2f !important; background: none; border: none; font-size: 1em; cursor: pointer; letter-spacing: 0.03em; padding: 0.45em 0.7em; border-radius: 8px; transition: text-decoration 0.11s; text-transform: none;}
.reset-filters-btn:hover, .reset-filters-btn:focus-visible { text-decoration: underline; background: none; outline: none;}
.active-sort-option { background: #e5f2fd; font-weight: 700; color: #2e9be6; border-radius: 6px;}
.active-sort-option:hover { background: #cbe5fb;}
@media (max-width: 900px) {
  .sortbar-tablet { padding: 0.5em 0.3em 0.3em 0.3em; border-radius: 12px; gap: 0.5em;}
  .tablet-row-1 { gap: 0.45em; flex-wrap: wrap;}
  .tablet-row-2 { gap: 0.55em; }
  .dropdown-btn, .reset-filters-btn, .hide-watched-btn { padding: 0.29em 0.8em; font-size: 1em; border-radius: 8px; min-width: 100px;}
  .search-bar-container { gap: 0.3em;}
  .search-input { width: 120px; font-size: 0.98em;}
  .switch-label-text { font-size: 0.98em;}
  .level-checkbox { font-size: 0.98em;}
}
@media (max-width: 600px) {
  .sortbar-tablet { padding: 0.18em 0.03em; gap: 0.2em;}
  .tablet-row-1, .tablet-row-2 { flex-direction: column; gap: 0.18em;}
  .dropdown-btn, .reset-filters-btn, .hide-watched-btn { min-width: 60px; font-size: 0.92em; width: 100%;}
  .search-bar-container, .search-input { width: 100%; max-width: 100vw; }
}
</style>
