<script>
  ...
  function handleCsvFile(e) { csvFile = e.target.files[0]; }
  async function uploadCsv() {
    if (!csvFile) return;
    bulkUploading = true; bulkResults = [];
    try {
      ...
      bulkUploading = false; bulkResults = out.results || [];
    } catch (err) {
      bulkUploading = false; bulkResults = [{ url: '', error: err.message || 'Bulk upload failed' }];
    }
  }
  ...
  async function fetchChannelDetails() {
    loading = true; error = ''; success = false; channelPreview = null;
    try {
      ...
      if (!res.ok) { error = data.error || 'Failed to fetch channel.'; return; }
      channelPreview = data.channel;
    } catch (err) { error = err.message || 'Fetch error'; }
    finally { loading = false; }
  }

  async function submitChannel() {
    error = ''; success = false;
    if (!level || !channelPreview?.id) { error = 'Level and channel data required.'; return; }
    try {
      const payload = {
        id: channelPreview.id, name: channelPreview.title, thumbnail: channelPreview.thumbnail,
        level, country, tags: selectedTags.join(', ')
      };
      const res = await fetch('/api/insert-channel', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
      });
      const data = await res.json();
      if (!res.ok) { error = data.error || 'Insert failed'; return; }
      success = true; url = ''; channelPreview = null; level = ''; country = ''; selectedTags = [];
    } catch (err) { error = err.message || 'Insert error'; }
  }
  ...
  function onSearchInput(e) { search = e.target.value; currentPage = 1; }
  function goToPage(p) { if (p < 1 || p > totalPages) return; currentPage = p; }
...
<!-- === AUTH & PROFILE DEBUG INFO === -->
<div style="background: #ffe6bf; color: #783100; font-size: 0.99em; padding: 0.5em 1em; margin-bottom:1.2em; border-radius: 8px;">
  <b>Auth Debug/Test</b><br>
  authChecked: {$authChecked ? 'true' : 'false'}<br>
  userLoading: {$userLoading ? 'true' : 'false'}<br>
  user: <pre>{JSON.stringify($user, null, 2)}</pre>
  <button on:click={reloadProfile} style="margin-top:0.7em;">Reload Profile</button>
  <div>
    profile:<br>
    <pre>{JSON.stringify($user?.profile, null, 2)}</pre>
    is_admin value: (type: {typeof $user?.profile?.is_admin}) {String($user?.profile?.is_admin)}
    <br>
    Admin status:
    {#if $user?.profile?.is_admin === true}
      <span style="color:green;">You are admin ✅</span>
    {:else if $user?.profile}
      <span style="color:red;">You are NOT admin ❌</span>
    {/if}
  </div>
</div>

<!-- === ADMIN PANEL ONLY IF ADMIN === -->
{#if $authChecked && !$userLoading && $user && $user.profile?.is_admin === true}
  <div class="container">
    <!-- === Bulk Upload CSV Bar === -->
...
{:else if $authChecked && !$userLoading && $user && $user.profile}
  <div style="margin:4em; color:#e93c2f; text-align:center;">Not allowed (admin only).</div>
{:else}
  <div style="margin:4em; text-align:center;">Loading / Checking auth...</div>
{/if}
