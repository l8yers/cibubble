...
console.log("add-channel: Top of file loaded");
...
console.log("add-channel: Env vars", { SUPABASE_URL, SUPABASE_ANON_KEY, YOUTUBE_API_KEY });
...
async function fetchVideoDurations(videoIds) {
  ...
  for (let i = 0; i < videoIds.length; i += 50) {
    ...
    try {
      ...
      console.log("add-channel: fetchVideoDurations error", err);
...
async function getChannelIdFromHandle(handle) {
  ...
  try {
    ...
    console.log("add-channel: getChannelIdFromHandle error", err);
    // Ignore errors
  }
  return null;
}
...
async function getChannelInfo(channelId) {
  ...
  try {
    ...
    console.log("add-channel: getChannelInfo error", err);
    return null;
  }
}
...
async function getPlaylists(channelId) {
  ...
  try {
    ...
    console.log("add-channel: getPlaylists error", err);
    // ignore
  }
...
async function getPlaylistVideos(playlistId) {
  ...
  try {
    ...
    console.log("add-channel: getPlaylistVideos error", err);
  }
...
export async function POST({ request }) {
  console.log("add-channel: POST handler started");
  try {
    if (!YOUTUBE_API_KEY) {
      console.log("add-channel: Missing YOUTUBE_API_KEY");
      return json({ error: 'Missing YouTube API key' }, { status: 500 });
    }

    const body = await request.json();
    const { url, tags, level, added_by, country } = body;
    console.log("add-channel: Request body", body);

    const tagArr = normalizeTags(tags);
    const normCountry = normalizeCountry(country);

    const extracted = extractChannelIdOrHandle(url);
    console.log("add-channel: Extracted channel info", extracted);
    if (!extracted) {
    ...
    if (!channelId && extracted.handle) {
      ...
      console.log("add-channel: Resolved channelId from handle", channelId);
      if (!channelId) {
    ...
    console.log("add-channel: Channel info", channel);
    if (!channel) {
    ...
    console.log("add-channel: Playlist videos gathered", playlistVideos.length);

    let uploadsVideos = [];
    if (channel.uploadsPlaylistId) {
      uploadsVideos = await getAllUploads(channel.uploadsPlaylistId);
      console.log("add-channel: Uploads videos gathered", uploadsVideos.length);
    }
    ...
    console.log("add-channel: All videos deduped", allVideos.length);
    ...
    if (allVideos.length > 0) {
      ...
      console.log("add-channel: Upserting videos", videosToUpsert.length);
      const { error: videoError } = await supabase.from('videos').upsert(videosToUpsert);
      if (videoError) {
        console.log("add-channel: Videos upsert error", videoError);
        return json({ error: 'Failed to upsert videos: ' + videoError.message }, { status: 500 });
      }
    }

    console.log("add-channel: Success, returning result");
    return json({
      success: true,
      channel,
    ...
    console.log("add-channel: Caught error", err);
