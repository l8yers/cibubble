	...
	<div class="profile-main">
		...
		{#if !showSettings}
			...
			<div class="progress-controls-row">
				...
				<button class="cibubble-btn" on:click={openManualModal} type="button">
					Add Time From Outside the Platform
				</button>
			</div>

			{#if dailyOpen}
				<ProgressDailyTotals {dailyTotals} {formatMinutesOnly} />
			{/if}

			{#if showManualModal}
				<div class="modal-backdrop" on:click={closeManualModal}>
					<div class="modal-content" on:click|stopPropagation>
						<button class="close-button" on:click={closeManualModal} aria-label="Close modal"
							>×</button
						>
						<ProgressManualEntry
							onAdded={() => {
			...
			{:else}
				<div class="history-link-row">
					<a class="history-link-mobile" href="/history"> View full watch history </a>
				</div>
			{/if}
		{:else}
			<ProgressSettings
