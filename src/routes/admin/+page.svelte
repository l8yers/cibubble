<script>
  ...
  onMount(async () => {
    ...
    // Show user id on page for debug
    ...
    if (!profile) {
      alert("No profile row found for user! Your user id is: " + user.id);
      goto('/login');
      return;
    }
    if (!profile.is_admin) {
      alert("User is not admin. id: " + user.id + " is_admin: " + profile.is_admin);
      goto('/');
      return;
    }
    isAdmin = true;
    loadingAdmin = false;
    alert("Access granted: admin panel shown.");
  });
  // --- Master lists ---
...
<!-- Debug banner so you know it's rendering! -->
<p style="color:red; font-size:2em;">ADMIN PANEL RENDERED</p>
