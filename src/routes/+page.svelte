<script>
  ...
  // --- State stores ---
  import {
    selectedChannel, selectedPlaylist, selectedLevels, sortBy,
    selectedCountry, selectedTags, hideWatched, watchedIds, searchTerm
  ...
  onMount(() => {
    mounted = true;
    function check() { isMobile.set(window.innerWidth <= 700); }
    check();
