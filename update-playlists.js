...
async function getAllPlaylists(channelId) {
  ...
  let nextPage = '';
  do {
    const url = `https://www.googleapis.com/youtube/v3/playlists?part=snippet,contentDetails&channelId=${channelId}&maxResults=50${nextPage ? `&pageToken=${nextPage}` : ''}&key=${YOUTUBE_API_KEY}`;
...
async function main() {
  // Get all channels that have ever been synced (change this query as needed for your workflow)
  const { data: channels, error } = await supabase
    .from('channels')
    .select('id, name, playlists_last_synced')
    .not('playlists_last_synced', 'is', null);
  ...
  for (const chan of channels) {
    try {
      ...
      // Get existing playlist records for this channel (to compare, optional)
      const { data: storedPlaylists } = await supabase
        .from('playlists')
        .select('id, updated_at')
        .eq('channel_id', chan.id);

      const storedMap = Object.fromEntries((storedPlaylists || []).map(p => [p.id, p]));

      let batch = [];
      let updatedAny = false;

      for (const pl of playlists) {
        // If playlist is new or possibly changed (could add smarter checks here with etag if you want)
        const isNew = !storedMap[pl.id];
        // Always upsert for simplicity, or add logic to check timestamps
      ...
      // Set last synced only if updated any
      if (updatedAny) {
        await supabase
          .from('channels')
